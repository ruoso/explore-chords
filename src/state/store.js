...
export function createStore(initial = {}) {
  ...
  const store = {
    ...
    /**
     * Which variation of a song's voicings is being read, on one tuning.
     *
     ...
     */
    chosenVariation(sheetId, tuning) {
      const chosen = state.prefs.variations?.[`${sheetId}|${normaliseTuning(tuning)}`];
      return chosen ?? null;
    },

    /**
     * The variation to read with: the one chosen, or the song's first for this tuning.
     *
     * The fallback matters because the choice lives outside the text: a variation can
     ...
     */
    activeVariation(sheetId, tuning, sets) {
      const stored = store.chosenVariation(sheetId, tuning);
      return sets.some((entry) => entry.name === stored) ? stored : (sets[0]?.name ?? null);
    },

    chooseVariation(sheetId, tuning, name) {
      const key = `${sheetId}|${normaliseTuning(tuning)}`;
      const variations = { ...(state.prefs.variations ?? {}) };
      if (name === null) delete variations[key];
      else variations[key] = name;
      return store.setPrefs({ variations });
    },
